...
const IssuesTracker = () => {
  ...
  const [userIssues, setUserIssues] = useState<UserIssues[]>([]);
  ...
  useEffect(() => {
    ...
    const fetchIssues = async () => {
      try {
        await axios
          .get(`http://localhost:3000/api/user/track/issues/`)
          .then((res) => {
            setUserIssues(res.data.userIssues);
  ...
  const filteredIssues = userIssues.filter((userIssue) => {
    const matchesSearch = userIssue.issue.title
      .toLowerCase()
      .includes(searchQuery.toLowerCase());
    const matchesRepo =
      selectedRepo === "all" ||
      `${userIssue.issue.owner}/${userIssue.issue.name}` === selectedRepo;
    const matchesLabel =
      selectedLabel === "all" ||
      userIssue.issue.labels.some(
        (label) => normalize(label) === selectedLabel
      );
        ...
        <div className="max-w-7xl mx-auto">
          <div className="mb-8">
            <div className="flex items-center justify-between mb-4">
              <h1 className="text-4xl font-bold">Tracked Issues</h1>
              <Button
                variant="outline"
                size="sm"
                className="gap-2"
